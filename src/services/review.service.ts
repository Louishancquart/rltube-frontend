...
export class ReviewService {
  ...
  // public fetchReviewList(): Review[] {
  //   return this.http.get(this.restUrl)
  //     .toPromise()
  //     // .then(response =>
  //     //   response.json() as Review[])
  //     .then(response => JSON.parse(JSON.stringify(response)))
  //     .then(res => console.log( "RESSSS:" + JSON.stringify(  JSON.parse(res["_body"]))) )
  ...
  public getReviewsByReviewedMediaUrl(mediaUrl: string): Promise<Review[]> {
    return this.http.get(this.restUrl + '/' + mediaUrl)
      ...
      .then(res => {
        for (const id in res) {
          this.reviewList.push(res[id] as Review);
        }
        console.log("video list LEN: " + this.reviewList.length);
        this.updateLists()
      })
      .catch(this.handleError);
  }

  public getReviewsById(videoID: string): Promise<Review[]> {
  ...
  getReviewList(mediaUrl: string): void {
    ...
    //   console.log("review service", "NOTHING TO DO")
